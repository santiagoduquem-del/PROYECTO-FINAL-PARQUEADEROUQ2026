...
public class GenerarReporteController {
    ...
    void onGenerarReporte(ActionEvent event) {
        ...
        List<Vehiculo> vehiculosHoy = DataHolder.getInstance().getVehicleList().stream()
                .filter(v -> v.getHoraIngreso() != null && v.getHoraIngreso().toLocalDate().equals(hoy))
                .collect(Collectors.toList());

        // 1. Total vehículos
        int total = vehiculosHoy.size();
        lblTotalVehiculos.setText("Total vehículos ingresados hoy: " + total);

        // 2. Ingresos generados
        double ingresos = vehiculosHoy.stream()
                .filter(v -> v.getEstado() == estadoVehiculo.SALIO)
                .mapToDouble(Vehiculo::getValorPagado)
                .sum();
        lblIngresosGenerados.setText(String.format("Ingresos generados hoy: $%.2f", ingresos));

        // 3. Tiempo promedio en HORAS
        double promedioMinutos = vehiculosHoy.stream()
        ...
        lblTiempoPromedio.setText(String.format("Tiempo promedio de permanencia: %.1f h", promedioHoras));

        // 4. Filtro por tiempo en HORAS
        ...
        if (!filtroStr.isEmpty()) {
            try {
                ...
                String lista = vehiculosHoy.stream()
                        .filter(v -> v.getEstado() == estadoVehiculo.SALIO && v.getHoraSalida() != null)
